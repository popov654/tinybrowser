...
public class HTMLDocument extends JSObject {
    ...
    class querySelectorFunction extends Function {
        ...
        public JSValue call(JSObject context, Vector<JSValue> args, boolean as_constr) {
            for (int i = 0; i < args.size(); i++) {
                ...
                CSSParser parser = new CSSParser(document);
                Node node = parser.documentQuerySelector(args.get(i).asString().getValue());
    ...
    class querySelectorAllFunction extends Function {
        ...
        public JSValue call(JSObject context, Vector<JSValue> args, boolean as_constr) {
            ...
            for (int i = 0; i < args.size(); i++) {
                ...
                CSSParser parser = new CSSParser(document);
                Vector<Node> nodes = parser.documentQuerySelectorAll(args.get(i).asString().getValue());
                for (Node node: nodes) {
    ...
    class createElementFunction extends Function {
        ...
        public JSValue call(JSObject context, Vector<JSValue> args, boolean as_constr) {
            ...
            if (root.size() > 0 && com.alstarsoft.tinybrowser.bridge.Mapper.get(root.get(0)) != null) {
                com.alstarsoft.tinybrowser.render.Block root_block = com.alstarsoft.tinybrowser.bridge.Mapper.get(root.get(0));
