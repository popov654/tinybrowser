...
public class HTMLElementProto extends JSObject {
    ...
    class appendChildFunction extends Function {
        ...
        public JSValue call(JSObject context, Vector<JSValue> args, boolean as_constr) {
            ...
            com.alstarsoft.tinybrowser.render.Block block = Mapper.get(newNode);
            boolean result = node.addChild(newNode);

            if (result) {
                block.builder.assignDocument(node);
                block.builder.processSpecialElement(node);
                com.alstarsoft.tinybrowser.render.Block parent_block = Mapper.get(node);
    ...
    class removeChildFunction extends Function {
        ...
        public JSValue call(JSObject context, Vector<JSValue> args, boolean as_constr) {
            ...
            com.alstarsoft.tinybrowser.render.Block block = Mapper.get(((HTMLNode)args.get(0)).node);
    ...
    class clickFunction extends Function {
        ...
        public JSValue call(JSObject context, Vector<JSValue> args, boolean as_constr) {
            ...
            Node node = element.node;
            if (node != null) {
                com.alstarsoft.tinybrowser.render.Block block = Mapper.get(node);
                block.doMouseClick();
            }
            return Undefined.getInstance();
        }
